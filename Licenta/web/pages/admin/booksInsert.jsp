					...
					<table id="formularContent">
						<tr>
							...
							<td><s:if test="sessionBook.get('book') != true">
									...
									<s:textfield id="title" name="title" value="%{sessionBook.title}" />
								</s:else></td>
							<td><s:fielderror>
									...
									<s:if test="sessionBook.get('book')">
										...
										<div>
											...
											<s:textfield id="authorFN" name="authorFN"
												value="%{sessionBook.get('authorList').get(0).getFirstName()}" />
											<label id="authorLNLabel">Last Name:</label>
											<s:textfield id="authorLN" name="authorLN"
												value="%{sessionBook.get('authorList').get(0).getLastName()}" />
										</div>
										<s:iterator value="sessionBook.get('authorList')" begin="1"
											status="part">
											<div>
												<label id="authorFNLabel">First Name:</label>
						...
						<tr>
							...
							<td><s:if test="sessionBook.get('book')">
									<input type="button"
										onclick="addInput('authors','<s:property value='sessionBook.get("size")'/>')"
						...
						<tr>
							...
							<td><s:if test="sessionBook.get('book')">
									<s:textfield id="series" name="series"
										value="%{sessionBook.get('series')}" />
								</s:if> <s:else>
									<s:textfield id="series" name="series" value="%{series}" />
								</s:else></td>
						</tr>
						<tr>
							<td><label>Edition:</label></td>
							<td><s:if test="sessionBook.get('book')">
									<s:textfield id="edition" name="edition"
										value="%{sessionBook.get('edition')}" />
								</s:if> <s:else>
									<s:textfield id="edition" name="edition" value="%{edition}" />
								</s:else></td>
						</tr>
						<tr>
							...
							<td><select name="month">
									...
									<s:iterator value="monthList" status="part">
										<s:if test="sessionBook.get('book')">
											<s:if
												test="%{sessionBook.get('month') == monthList.get(#part.index)}">
